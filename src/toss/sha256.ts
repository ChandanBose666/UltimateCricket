/**
 * SHA-256, pure TypeScript, no dependencies.
 *
 * Written by hand because §2 of the plan closes the dependency list and because
 * `crypto.subtle` is async and not reliably present under Hermes / React Native
 * Web. Verified against the published FIPS 180-4 vectors in __tests__/sha256.test.ts —
 * if those pass, the "provably fair toss" claim on stage is honest.
 *
...
/** UTF-8 encode. Exported so the encoder can be tested on its own. */
export function utf8Bytes(str: string): number[] {
  ...
  for (let i = 0; i < str.length; i++) {
    ...
    if (c < 0x80) {
      ...
      // Lone high surrogate — encode it as-is rather than throwing.
...
export function sha256Hex(message: string): string {
  ...
  const bitsHi = Math.floor(len / 0x20000000); // = (len * 8) >>> 32
